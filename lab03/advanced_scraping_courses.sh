...
elif test $year -le 2018
    undergraduate_legacy="http://legacy.handbook.unsw.edu.au/vbook$year/brCoursesByAtoZ.jsp?StudyLevel=Undergraduate&descr=$code"
    postgraduate_legacy="http://legacy.handbook.unsw.edu.au/vbook$year/brCoursesByAtoZ.jsp?StudyLevel=Postgraduate&descr=$code"

    curl -s $undergraduate_legacy $postgraduate_legacy | egrep "^<TD class=\"[a-zA-Z]*\"><A herf=\"[0-9a-zA-Z\/.]*\">"

# advanced part are based on the scraping_courses.sh
